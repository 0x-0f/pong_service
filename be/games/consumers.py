...
class PongQueueConsumer(AsyncWebsocketConsumer):
	async def connect(self):
		...
		async with pong_queue_lock:  # lock 걸고 큐 확인
			if self.user_id in pong_queue: # 동일 intra id 로 웹소켓 연결 거절
				self.status = "denied" # 거절 상태 저장
				await self.close()
				return
			else:
				await self.accept()  # 웹소켓 연결 수락 후 대기 큐에 등록
				pong_queue.append(self.user_id)
	...
	async def disconnect(self, close_code):
		...
		if self.status == "waiting":  # 대기중 매치가 성사되지 않은 상태에서 퇴장(연결종료). 대기큐에서 제거
			async with pong_queue_lock:
				if self.user_id in pong_queue:
					pong_queue.remove(self.user_id)
		elif self.user_id == self.user1_user_id:  # user1 인지 확인 (대기열에서 삭제는 user1 에서만 진행)
			while True:
				async with pong_cross_lock:  # 매칭에 lock 걸고 서로의 아이디를 확인했는지 확인
					if pong_cross_check == 2:
						async with pong_queue_lock:  # 서로 확인했다면 lock 걸고 대기큐에서 삭제 (양쪽 모두)
							if self.user1_user_id in pong_queue:
								pong_queue.remove(self.user1_user_id)
							if self.user2_user_id in pong_queue:
								pong_queue.remove(self.user2_user_id)
	...
	async def wait_for_match(self):
		...
		while True:  # 매칭 대기 루프
			async with pong_queue_lock:
				index = pong_queue.index(self.user_id)
				length = len(pong_queue)
			if length > 1 and (index == 0 or index == 1): # 게임 매칭 조건: queue의 길이가 2 이상이면서, 나의 index가 0 or 1 이면 매칭가능
				async with pong_queue_lock:
					self.user1_user_id = pong_queue[0]
					self.user2_user_id = pong_queue[1]
				async with pong_cross_lock:  # lock 걸고 cross check +1 (상대방 아이디를 확인했다는 표시)
					pong_cross_check += 1
				await self.send(json.dumps({"match_url": f"/ws/pong/match/{self.user1_user_id}_{self.user2_user_id}/"})) # user_id 조합하여 새 웹소켓 URL 생성 및 보내기
				self.status = "matched"
				await self.close()
...
class RPSQueueConsumer(AsyncWebsocketConsumer):
	async def connect(self):
		...
		async with rps_queue_lock:
			if self.user_id in rps_queue: # 중복된 사용자 요청은 거절
				self.status = "denied" # 거절상태 저장
				self.close()
				return
			else:
				await self.accept()  # 웹소켓 연결 수락 및 대기큐 등록
				rps_queue.append(self.user_id)
	...
	async def disconnect(self, close_code):
		...
		if self.status == "waiting":  # 매칭이 안된 상태에서 종료
			async with rps_queue_lock:
				if self.user_id in rps_queue:
					rps_queue.remove(self.user_id)  # 매칭이 안되고 대기열에서 삭제 후 종료
		elif self.user_id == self.user1_user_id:  # user1 인지 확인 (대기열에서 삭제는 한쪽에서만 진행)
			while True:
				async with rps_cross_check_lock:  # 매칭에 lock 걸고 서로의 아이디를 확인했는지 확인
					if rps_cross_check == 2:
						async with rps_queue_lock:  # 서로 확인했다면 lock 걸고 대기큐에서 삭제 (양쪽 모두)
							if self.user1_user_id in rps_queue:
								rps_queue.remove(self.user1_user_id)
							if self.user2_user_id in rps_queue:
								rps_queue.remove(self.user2_user_id)
	...
	async def wait_for_match(self):
		...
		while True:   # 매칭 대기 루프: queue의 길이가 2 이상이면서, 나의 index가 0 or 1 이면 매칭가능
			async with rps_queue_lock:
				index = rps_queue.index(self.user_id)
				length = len(rps_queue)
			if length > 1 and (index == 0 or index == 1):
				self.status = "matched"
				async with rps_queue_lock:     # lock 걸고 매칭된 user_id 가져오기
					self.user1_user_id = rps_queue[0]
					self.user2_user_id = rps_queue[1]

				async with rps_cross_check_lock:  # lock 걸고 크로스 체크 +1
					rps_cross_check += 1
				await self.send(json.dumps({"match_url": f"/ws/rps/match/{self.user1_user_id}_{self.user2_user_id}/"})) # user_id 조합하여 새 웹소켓 URL 생성 및 보내기
				await self.close()
...
class RPSMatchConsumer(AsyncWebsocketConsumer):
	async def connect(self):
		...
		async with asyncio.Lock():  # lock 걸고 내 역할을 지정 및 내 상태를 off->on 으로 활성화
			...
			rps_game_rooms[self.match_name].user_id[self.role] = self.user_id
